  ...
  document.getElementById('productDetail').innerHTML = `
    <div class="product-gallery">
      <div class="product-main-image" id="productMainImage">
        ${images.length > 0 ? `<img src="${images[0]}" alt="${product.name}" id="mainImg">` : '<div style="color:var(--sage-700)">No image</div>'}
        ${product.video ? `<video src="${product.video}" controls style="display:none;width:100%;border-radius:10px" id="mainVideo"></video>` : ''}
      </div>
      ${images.length > 1 ? `
        <div class="product-thumbs">
          ...
          `).join('')}
          ${product.video ? `
            <div class="product-thumb" onclick="playVideo()" style="display:flex;align-items:center;justify-content:center;background:var(--sage-700);color:white">
              ▶ Video
            </div>
          ` : ''}
        </div>
...
function playVideo() {
  ...
  if (mainVideo) { mainVideo.style.display = 'block'; mainVideo.play(); }
  document.querySelectorAll('.product-thumb').forEach(t => t.classList.remove('active'));
}
